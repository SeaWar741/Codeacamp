...
#   user = TwitterUser.new(email: params[:user_email], password: params[:user_password])
...
# post '/login' do
#   user = TwitterUser.authenticate(params[:user_email], params[:user_password])
#   if user
#     session[:id] = user.id
#     redirect to ("/users/#{user.id}")
#   else
...
#   tweet = Tweet.find_by(text: params["user_tweet"])
#   if tweet == nil
#    Tweet.create(twitter_user_id: session[:id], text: params["user_tweet"])
#     CLIENT.update(params["user_tweet"])
#   else
#     CLIENT.update(params["user_tweet"])
#   end
# end
...
#   p redirect request_token.authorize_url(:oauth_callback => "http://#{host_and_port}/auth")
